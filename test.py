...
def keygen(offset):
    ...
    pk = (a, b)
    sk = s  # Секретный ключ сохраняется только для деккапсуляции
    return pk, sk

def encapsulate(pk, offset, threshold):
    """
    Инкапсуляция: передаем только публичные параметры и закодированное сообщение.
    ...
    s_prime = sample_secret() # Эфемерный секретный ключ для инкапсуляции
...
def decapsulate(ciphertext, sk, offset):
    """
    Декапсуляция: только на стороне получателя используется секретный ключ для восстановления сообщения.
    """
    u, v, hint = ciphertext
